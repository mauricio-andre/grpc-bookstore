...
function GetAuthor(
  call: ServerUnaryCall<GetAuthorRequest, GetAuthorResponse>,
  callback: sendUnaryData<GetAuthorResponse>,
  database: DB,
) {
  try {
...
function UpdateAuthor(
  call: ServerUnaryCall<UpdateAuthorRequest, UpdateAuthorResponse>,
  callback: sendUnaryData<UpdateAuthorResponse>,
  database: DB,
) {
  try {
    assertAuthor(database, call.request.getAuthorid());
    const newAuthor = database.updateAuthor(
      call.request.getAuthorid(),
...
function DeleteAuthor(
  call: ServerUnaryCall<DeleteAuthorRequest, object>,
...
function ListAuthor(
  _: ServerUnaryCall<object, ListAuthorResponse>,
  callback: sendUnaryData<ListAuthorResponse>,
  database: DB,
) {
  return callback(
    null,
    new ListAuthorResponse().setAuthorsList(database.listAuthors()),
  );
}

function CreateAuthor(
  call: ServerUnaryCall<CreateAuthorRequest, CreateAuthorResponse>,
  callback: sendUnaryData<CreateAuthorResponse>,
  database: DB,
) {
  try {
