...
class DB {
  readonly #dbPath = path.resolve(__dirname, '../db/db.json');
  ...
  constructor(dbPath = null, initialData: null | {} = null) {
    if (dbPath) {
      this.#dbPath = dbPath;
    }
  ...
  load(initialData) {
    const readData = initialData
      ? initialData
      : JSON.parse(readFileSync(this.#dbPath, 'utf8'));
  ...
  addBook(book: CreateBookRequest.CreateData) {
    const id = crypto
      .createHash('sha256')
    ...
    if (this.getBook(id))
      throw new Error(
        `Book "${book.getAuthor()}" by "${this.getAuthor(
          book.getAuthor(),
  ...
  addAuthor(author: CreateAuthorRequest.CreateData) {
    const id = crypto
      .createHash('sha256')
  ...
  listAuthors() {
    return [...this.data.authors.values()].map(doc =>
      this.fromObject(new Author(), doc),
    ) as Author[];
  }
  ...
  private fromObject(
    ...
    document?: Book.AsObject | Author.AsObject,
  ...
  private removeEmptyKeys(
    obj: UpdateAuthorRequest.UpdateData | UpdateBookRequest.UpdateData,
  ) {
    const clonedData = Object.assign({}, obj.toObject());

    Object.keys(clonedData).forEach(
      key => !clonedData[key] && delete clonedData[key],
    );
